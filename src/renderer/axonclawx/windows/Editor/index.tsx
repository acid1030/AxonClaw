...
const BASIC_SECTION_IDS: SectionId[] = ['models', 'channels', 'gateway', 'auth', 'memory', 'extensions'];
...
const Editor: React.FC<EditorProps> = ({ language, pendingSection, onSectionConsumed, minimal = false }) => {
  const resolveInitialSection = (): SectionId => {
    if (pendingSection && SECTIONS.some((s) => s.id === pendingSection)) {
      return pendingSection as SectionId;
    }
    return 'models';
  };
  ...
  const filteredSections = useMemo(() => {
    let list = SECTIONS;
    if (!advancedMode) {
      const basicIds = new Set<SectionId>(BASIC_SECTION_IDS);
      list = list.filter((s) => basicIds.has(s.id));
    }
  ...
  }, [advancedMode, searchQuery, es, unmappedCount]);

  useEffect(() => {
    if (filteredSections.some((s) => s.id === activeSection)) return;
    const fallback = filteredSections[0]?.id || 'models';
          ...
          <button
            ...
            className={`relative w-10 h-5 rounded-full border transition-colors ${
              advancedMode
                ...
                : 'bg-slate-300 dark:bg-white/15 border-slate-300 dark:border-white/10'
            }`}
            title={ed.advancedMode || 'Advanced Mode'}
          >
            <span
              className={`absolute left-0 top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${
                advancedMode ? 'translate-x-[20px]' : 'translate-x-[2px]'
              }`}
            />
          </button>
        </div>
