...
export function useConfigEditor(): UseConfigEditorReturn {
  ...
  // Default to local so Setup Wizard writes (to ~/.openclaw/openclaw.json)
  // are immediately visible in Config Center.
  ...
  const load = useCallback(async () => {
    ...
    try {
      let data: any;
      if (mode === 'local') {
        // Local mode: prefer ~/.openclaw/openclaw.json as single source of truth.
        try {
          data = await configApi.get();
          console.log('[ConfigEditor] local configApi.get() response:', JSON.stringify(data, null, 2).slice(0, 500));
          if (data?.path) setConfigPath(data.path);
        } catch (localErr: any) {
          console.log('[ConfigEditor] local configApi.get() failed:', localErr?.message || localErr);
          const errCode = localErr?.code || (localErr?.message?.includes('not found') ? 'CONFIG_NOT_FOUND' : '');
          if (errCode) setLoadErrorCode(errCode);
          // Fallback to gateway only when local file read fails.
          try {
            ...
            throw new Error(errCode === 'CONFIG_NOT_FOUND' ? 'Config file not found' : 'Failed to load local config');
          }
        }
      } else {
        // Remote mode: prefer gateway, fallback to local file.
        try {
          data = await gwApi.configGet();
          if (typeof data === 'string') {
