...
const Doctor: React.FC<DoctorProps> = ({ language }) => {
  ...
  const jumpToWindow = useCallback((id: string, opts?: { section?: string }) => {
    window.dispatchEvent(new CustomEvent('axonclaw:open-window', { detail: { id, ...opts } }));
  }, []);
                          ...
                          <div className="mt-1.5 pt-1.5 border-t border-slate-100 dark:border-white/5">
                            <div className="flex items-center gap-1 flex-wrap">
                              ...
                              {relatedFaqs.map(faq => (
                                <button
                                  key={faq.id}
                                  onClick={() => window.dispatchEvent(new CustomEvent('axonclaw:open-window', { detail: { id: 'knowledge', expandItem: faq.id } }))}
