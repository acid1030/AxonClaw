...
const UpdateTab: React.FC<UpdateTabProps> = ({ s, language, inputCls, rowCls }) => {
  ...
  // ── 服务状态 ──
  const [serviceStatus, setServiceStatus] = useState<{ openclaw_installed: boolean; axonclawx_installed: boolean } | null>(null);
  ...
  const handleTranslateNotes = useCallback(async (text: string, product?: string, ver?: string) => {
    ...
    try {
      const res = await selfUpdateApi.translateNotes(text, language, product || 'axonclawx', ver || '0');
  ...
  const handleServiceInstall = useCallback(async (service: 'openclaw' | 'axonclawx') => {
    setServiceLoading(true);
    try {
      if (service === 'openclaw') {
        const res = await gatewayApi.daemonInstall();
        // Immediately reflect the installed state from the response
        setServiceStatus(prev => prev ? { ...prev, openclaw_installed: res.installed } : { openclaw_installed: res.installed, axonclawx_installed: false });
  ...
  const handleServiceUninstall = useCallback(async (service: 'openclaw' | 'axonclawx') => {
    setServiceLoading(true);
    try {
      if (service === 'openclaw') {
                  ...
                  <div className="flex items-center gap-1">
                    {language !== 'en' && (
                      translatedNotes ? (
                        ...
                        <button onClick={() => handleTranslateNotes(selfUpdateInfo.releaseNotes!, 'axonclawx', selfUpdateInfo.latestVersion || selfUpdateInfo.currentVersion)} disabled={notesTranslating}
        ...
        <div className="px-5 py-4">
          ...
          {serviceStatus ? (
            <div className="space-y-2">
              ...
              {/* AxonClawX Service */}
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-colors ${
                serviceStatus.axonclawx_installed
                  ? 'bg-emerald-50 dark:bg-mac-green/5 border-emerald-200 dark:border-mac-green/20'
                  : 'bg-slate-50 dark:bg-white/[0.02] border-slate-200 dark:border-white/[0.06]'
              }`}>
                <span className="text-[16px]">🦀</span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => serviceStatus.axonclawx_installed ? handleServiceUninstall('axonclawx') : handleServiceInstall('axonclawx')}
                    disabled={serviceLoading}
                    className={`flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] font-bold transition-colors disabled:opacity-40 ${
                      serviceStatus.axonclawx_installed
                        ? 'bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20'
                  ...
                  >
                    <span className={`material-symbols-outlined text-[13px] ${serviceLoading ? 'animate-spin' : ''}`}>
                      {serviceLoading ? 'progress_activity' : serviceStatus.axonclawx_installed ? 'delete_forever' : 'install_desktop'}
                    </span>
                    {serviceStatus.axonclawx_installed ? (s.uninstallService || 'Remove') : (s.installService || 'Install')}
                  </button>
                </div>
              </div>
            </div>
