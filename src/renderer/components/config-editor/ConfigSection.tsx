...
export const ConfigSection: React.FC<ConfigSectionProps> = ({
  ...
  return (
    <div
      className={cn(
        'bg-slate-50/80 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl transition-colors',
    ...
    >
      <div
        className={cn(
          ...
          collapsible && 'cursor-pointer hover:bg-slate-100 dark:hover:bg-white/[0.03]'
        )}
      ...
      {open && (
        <div className="px-3 md:px-4 pb-3.5 md:pb-3 border-t border-slate-100 dark:border-white/[0.04]">
          {children}
        </div>
      )}
    </div>
